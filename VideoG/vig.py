...
class Stem(nn.Module):
    ...
    def __init__(self, img_size=224, in_dim=3, out_dim=768, act='relu'):
        ...
        self.convs = nn.Sequential(
            nn.Conv3d(in_dim, out_dim//8, 3, stride=(1, 2, 2), padding=(1, 1, 1)),
            nn.BatchNorm3d(out_dim//8),
            act_layer(act),
            nn.Conv3d(out_dim//8, out_dim//4, 3, stride=(1, 2, 2), padding=(1, 1, 1)),
            nn.BatchNorm3d(out_dim//4),
            act_layer(act),
            nn.Conv3d(out_dim//4, out_dim//2, 3, stride=(1, 2, 2), padding=(1, 1, 1)),
            nn.BatchNorm3d(out_dim//2),
            act_layer(act),
            nn.Conv3d(out_dim//2, out_dim, 3, stride=(1, 2, 2), padding=(1, 1, 1)),
            nn.BatchNorm3d(out_dim),
            act_layer(act),
            nn.Conv3d(out_dim, out_dim, 3, stride=(1, 1, 1), padding=(1, 1, 1)),
            nn.BatchNorm3d(out_dim),
        )

    def forward(self, x):
        x = self.convs(x) # (N, 320, 16, 14, 14)
        B, C, T, H, W = x.shape
        assert T == 16
        # x = rearrange(x, 'b c (t1 t2) h w -> b c t1 t2 h w', t1=4)
        # x = rearrange(x, 'b c t1 t2 h w -> b c (h t1) (w t2)')
        return x
...
class DeepGCN(torch.nn.Module):
    def __init__(self, opt):
        ...
        # self.pos_embed = nn.Parameter(torch.zeros(1, channels, 14 * 4, 14 * 4))
        self.pos_embed = nn.Parameter(torch.zeros(1, channels, 16, 14 , 14 ))
        ...
        # self.framepass = Seq(nn.Conv3d(channels, 1024, 1, bias=True),
        #                       nn.BatchNorm3d(1024),
        #                       act_layer(act),
        #                       nn.Dropout(opt.dropout),
        #                       nn.Conv3d(1024, opt.n_classes, 1, bias=True))

        self.prediction = Seq(nn.Conv3d(channels, 1024, (4,7,7), stride=(4, 7, 7), bias=True),
                              nn.BatchNorm3d(1024),
                              act_layer(act),
                              nn.Dropout(opt.dropout),
                              nn.Conv3d(1024, 1024, (4,2,2), stride=(4, 2, 2), bias=True),
                              nn.BatchNorm3d(1024),
                              act_layer(act),
                              nn.Dropout(opt.dropout),
                              nn.Conv3d(1024, opt.n_classes, (1,1,1), stride=(1, 1, 1),bias=True))
        self.model_init()
...
#         x = F.adaptive_avg_pool3d(x, (4,1,1)) 
        out = self.prediction(x).squeeze(-1).squeeze(-1).squeeze(-1)
        return out
...
def vig_ti_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=51, drop_path_rate=0.0, drop_rate=0.0, num_knn=9, **kwargs):
