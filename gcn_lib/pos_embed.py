...
def get_3d_relative_pos_embed(embed_dim, grid_depth, grid_height, grid_width):
    pos_embed = get_3d_sincos_pos_embed(embed_dim, grid_depth, grid_height, grid_width)
    relative_pos = 2 * np.matmul(pos_embed, pos_embed.transpose()) / pos_embed.shape[1]
    return relative_pos

def get_3d_sincos_pos_embed(embed_dim, grid_depth, grid_height, grid_width, cls_token=False):
    ...
    grid = grid.reshape([3, 1, grid_depth, grid_height, grid_width])
...
def get_1d_sincos_pos_embed_from_grid(embed_dim, pos):
    ...
    omega = np.arange(embed_dim, dtype=np.float)
    omega /= embed_dim
    omega = 1. / 10000**omega
